...
type NestedRecord = { [key: string]: string | boolean | number | NestedRecord }

function buildTree(data: Record<string, string>): NestedRecord {
...
function stringifyTree(tree: NestedRecord, indent = 1): string {
	...
	for (let i = 0; i < entries.length; i++) {
		...
		if (typeof value === "string") {
			result += row(`${key}: ${JSON.stringify(value)},`, indent)
		} else if (typeof value === "number" || typeof value === "boolean") {
			result += row(`${key}: ${value},`, indent)
		} else {
			result += row(`${key}: {`, indent)
			result += stringifyTree(value, indent + 1)
			result += row("},", indent)
		}
	}
...
async function saveLocaleFile(locale: locale | "EN", data: Record<string, string>) {
	...
	let fileContent = `// This file is auto-generated by tools/updateData/generateLocale.ts\n`
	fileContent += `import type { localeData } from "./index"\n`
	fileContent += `export default {\n`
	fileContent += stringifyTree(tree)
	fileContent += `} satisfies localeData as localeData\n`

	await writeFile(`./src/data/locale/${locale}.ts`, fileContent, "utf-8")
}
