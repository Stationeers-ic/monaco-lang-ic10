...
export const language: languages.IMonarchLanguage = {
	...
	tokenizer: {
		...
		instruction: [
			{ include: '@whitespace' },
			[/[a-zA-Z_]\w*:/, { token: 'support.type', next: '@pop', log: 'found number $0 in state $S0' }],
			[
				/[a-zA-Z_]\w*/,
				{
					cases: {
		...
		define: [
			{ include: '@whitespace' },
			[/[a-zA-Z_]\w*/, 'constant.numeric', '@popall'],
		],
		alias: [
			{ include: '@whitespace' },
			[/[a-zA-Z_]\w*/, 'variable', '@popall'],
		],
		label: [
			{ include: '@whitespace' },
